...
import { Send, X, Mail, TrendingUp, BarChart2 } from 'lucide-react';
...
export default function AuraAIChat() {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'ai'; content: string }[]>([
    { role: 'ai', content: "Hi! I'm AuraAI 👋 Ask me about your portfolio performance, risk, or investment strategy." },
  ]);
  ...
  const chips = [
    ...
    { icon: TrendingUp, label: 'Portfolio Risk', action: () => setInput('Analyze my portfolio risk'), color: 'text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20' },
    { icon: BarChart2, label: 'Top Performers', action: () => setInput('Show top performers'), color: 'text-violet-400 bg-violet-500/10 hover:bg-violet-500/20' },
  ];
          ...
          <form onSubmit={handleSubmit} className="px-4 py-3 border-t border-white/8 flex-shrink-0">
            <div className="flex gap-2">
              <input
                ...
                placeholder="Ask about portfolio, risk, or strategy..."
                disabled={isLoading}
                className="flex-1 px-3.5 py-2.5 bg-white/8 border border-white/10 rounded-xl text-sm text-white caret-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-primary/60 focus:border-primary/40 transition-all duration-200"
