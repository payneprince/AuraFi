...
export function UserDetailsModal({ isOpen, onClose, user }: UserDetailsModalProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl shadow-black/50 overflow-hidden animate-in slide-in-from-bottom-4 zoom-in-95 duration-300 [transition-timing-function:cubic-bezier(0.16,1,0.3,1)]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="relative p-6 gradient-primary text-white overflow-hidden">
        ...
        <div className="p-6 space-y-1">
          {details.map((d, i) => {
            ...
            return (
              <div
                key={d.label}
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-accent transition-colors animate-in fade-in slide-in-from-bottom-2 fill-mode-both duration-300"
        ...
        <div className="p-6 pt-0">
          <button
            onClick={onClose}
            className="w-full px-4 py-2.5 rounded-xl bg-muted text-foreground font-medium hover:bg-accent transition-colors"
          >
            Close
          </button>
        </div>
